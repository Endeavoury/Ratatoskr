...
- Artifact workspace: `.agentic/workflows/<work-id>/agents/compatibility-reviewer/<assignment-id>/`.
- Workspace outputs: `compatibility-report.md`, G12 record.
- Eligible shared paths, only when explicitly assigned: None; ABI, source and documentation are read-only.
...
Allowed, only within the delegation's concrete paths: Assigned workspace compatibility-report.md, gate record and compatibility handoff/completion records.
